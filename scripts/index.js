...
// Create dual joystick implementation for movement and shooting
class Joystick {
  constructor(id, parent, onMoveCallback) {
    this.id = id;
    ...
    this.thumb.id = `${id}-thumb`;
    this.thumb.style.position = "absolute";
    this.thumb.style.width = "60px";
    this.thumb.style.height = "60px";
    this.thumb.style.background = "rgba(255, 255, 255, 0.8)";
    ...
    // Event listeners
    this.container.addEventListener("touchstart", (e) => this.onStart(e.touches[0]));
  ...
  onMove(event) {
    ...
    this.x = Math.cos(angle) * (distance / 50);
    this.y = Math.sin(angle) * (distance / 50);

    this.thumb.style.left = `${50 + this.x * 50}%`;
    this.thumb.style.top = `${50 + this.y * 50}%`;
  ...
  onEnd() {
    ...
    this.thumb.style.left = "50%";
    this.thumb.style.top = "50%";
    if (this.onMoveCallback) this.onMoveCallback(this.x, this.y);
  }
}

// Initialize dual joysticks
...
const moveJoystick = new Joystick("moveJoystick", joystickContainer, (x, y) => {
...
const shootJoystick = new Joystick("shootJoystick", joystickContainer, (x, y) => {
